...
def build_random_matrix(word2idx, embed_dim, type):
    random_matrix_file_name = '{0}_{1}_random_matrix.pkl'.format(str(embed_dim), type)
    if os.path.exists(random_matrix_file_name):
        print('loading embedding_matrix:', random_matrix_file_name)
        embedding_matrix = pickle.load(open(random_matrix_file_name, 'rb'))
    else:
        print('loading vertors...')
        embedding_matrix = np.zeros((len(word2idx),embed_dim))
        embedding_matrix[1, :] = np.random.uniform(-1 / np.sqrt(embed_dim), 1 / np.sqrt(embed_dim), (1, embed_dim))
        print('building matrix')
        pickle.dump(embedding_matrix,open(random_matrix_file_name,'wb'))
    return embedding_matrix
...
class ABSADatesetReader:
    ...
    def __read_data__(fname, tokenizer):
        ...
        fin_asp = open(fname + '.asp_graph', 'rb')
        fin_asp2 = open(fname + '.asp2_graph', 'rb')
        fin_asp3 = open(fname + '.asp3_graph', 'rb')
        idx2gragh = pickle.load(fin)
        idx2asp_graph = pickle.load(fin_asp)
        idx2asp_graph_2 = pickle.load(fin_asp2)
        idx2asp_graph_3 = pickle.load(fin_asp3)
        fin.close()
        fin_asp.close()
        fin_asp2.close()
        fin_asp3.close()
        ...
        for i in range(0, len(lines), 3):
            ...
            asp_graph = idx2asp_graph[i]
            asp_graph_2 = idx2asp_graph_2[i]
            asp_graph_3 = idx2asp_graph_3[i]
            ...
            data = {
                ...
                'asp_graph':asp_graph,
                'asp_graph2':asp_graph_2,
                'asp_graph3':asp_graph_3
            }
    ...
    def __init__(self, dataset='twitter', embed_dim=300):
        ...
        #self.random_matrix = build_random_matrix(tokenizer.word2idx, embed_dim, dataset)
