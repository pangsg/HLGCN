...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        for item in batch_data:
            text_indices, context_indices, aspect_indices, left_indices, polarity, dependency_graph, asp_graph,asp_graph2,asp_graph3 = \
                item['text_indices'], item['context_indices'], item['aspect_indices'], item['left_indices'],\
                item['polarity'], item['dependency_graph'],item['asp_graph'],item['asp_graph2'],item['asp_graph3']
            ...
            batch_asp_graph.append(numpy.pad(asp_graph, \
                ((0, max_len - len(text_indices)), (0, max_len - len(text_indices))),'constant'))
            batch_asp_graph2.append(numpy.pad(asp_graph2, \
                                             ((0, max_len - len(text_indices)), (0, max_len - len(text_indices))),
                                             'constant'))
            batch_asp_graph3.append(numpy.pad(asp_graph3, \
                                             ((0, max_len - len(text_indices)), (0, max_len - len(text_indices))),
                                             'constant'))
        return { \
                ...
                'asp_graph': torch.tensor(batch_asp_graph), \
                'asp_graph2': torch.tensor(batch_asp_graph2), \
                'asp_graph3': torch.tensor(batch_asp_graph3), \
            }
